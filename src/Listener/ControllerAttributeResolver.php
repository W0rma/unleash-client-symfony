...
final class ControllerAttributeResolver implements EventSubscriberInterface
{
    ...
    public function onControllerResolved(ControllerEvent $event): void
    {
        ...
        $attributes = array_merge(method_exists($reflectionClass, 'getAttributes') ? $reflectionClass->getAttributes(ControllerAttribute::class, ReflectionAttribute::IS_INSTANCEOF) : [], method_exists($reflectionMethod, 'getAttributes') ? $reflectionMethod->getAttributes(ControllerAttribute::class, ReflectionAttribute::IS_INSTANCEOF) : []);
